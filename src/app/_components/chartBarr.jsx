...
export function ChartBarState({ context }) {
    ...
    return (
        <Card className="rounded-3xl shadow-sm border-slate-100 bg-white overflow-hidden transition-all hover:shadow-md flex flex-col h-full">
            <CardHeader className="flex flex-row items-center justify-between py-6 border-b border-slate-50 bg-slate-50/30">
                <div className="grid gap-1">
                ...
                <Select value={selectedYear} onValueChange={setSelectedYear}>
                    <SelectTrigger className="w-[110px] rounded-xl border-slate-200 bg-white font-semibold text-slate-600 focus:ring-emerald-500/10">
                        <SelectValue placeholder="Ano" />
                    </SelectTrigger>
